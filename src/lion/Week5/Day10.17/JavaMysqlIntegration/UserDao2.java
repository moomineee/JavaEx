...
public class UserDao2 {
    public void add() throws SQLException, ClassNotFoundException {
        ...
        ps.setString(1, "1");   // 쿼리를 입력하는 것
        ps.setString(2, "KiMoon");
        ps.setString(3, "1123");

        int status = ps.executeUpdate(); // workbench의 ctrl+enter.(데이터 insert) = 쿼리 실행
        // SQL Query문이 INSERT, DELETE, UPDATE일 경우 사용. 반환값의 타입은 INT형.
        // executeQuery는 SQL Query문이 SELECT의 경우 사용.

        ps.close(); // connection, Statement, Resultset에 대해 close를 해줘야 한다. 여기서는 connection close.
        con.close();
    }
